...
void RectangleFinder::detectRectangles(cv::Mat& image){
	...
	cv::inRange(tmpImgRGB, cv::Scalar(0, 0, 90), cv::Scalar(80, 80, 255), filtered);

	std::vector<std::vector<cv::Point> > squares;
	findSquares(filtered, squares);
...
void RectangleFinder::findSquares( const cv::Mat& image, std::vector<std::vector<cv::Point> >& squares){
    squares.clear();
    ...
    cv::findContours(gray0, contours, CV_RETR_LIST, CV_CHAIN_APPROX_NONE);
